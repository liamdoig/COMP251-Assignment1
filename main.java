...
public class main {
    ...
    public static void main(String[] args) {
        ...
        for (int n : nList) {
            ...
            int chainSum=0;
            for (i=0; i < n; i++) {
            	chainSum += MyChainTable.insertKey(keysToInsert[i]); //inserting keys while keeping track of collisions sum
            }
            ...
            //System.out.println("avgColListChain: "+avColListChain);

            int j;
            int probeSum=0;
            for (j=0; j < n; j++) {
            	probeSum += MyProbeTable.insertKey(keysToInsert[j]); // tracking probe collisions
            }
 ...
 /* In this exercise, you apply your removeKey method on an example.
        ...
        //ADD YOUR CODE HERE
        Open_Addressing MyProbeTable2 = new Open_Addressing(w, 137); //initializing new probe table
        int n = 16;
        ...
        for (int i=0; i < n; i++) {									 				 // removing keys 
        	removeIndex.add((double) i);								 // storing remove index 	
        	removeCollisions.add((double) MyProbeTable2.removeKey(keysToRemove[i])); // storing collisions on remove 
        }
